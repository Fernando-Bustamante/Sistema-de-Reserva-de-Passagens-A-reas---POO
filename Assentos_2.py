...
num_rows = 30
cols_layout = [3, 3]  # Definindo a estrutura 3 - 3
...
for section_index, section in enumerate(cols_layout):
    for row in range(num_rows):
        for col in range(section):
            absolute_col_index = sum(cols_layout[:section_index]) + col + section_index
            if absolute_col_index < len(column_titles):
                ...
                button = tk.Button(canvas, image=photo, width=image_width, height=image_height,
                                   ...
                                   command=lambda r=row+1, c=column_titles[absolute_col_index]: on_seat_click(r, c))
